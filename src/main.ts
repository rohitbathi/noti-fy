...
// Uncomment the following line to use the Google Cloud Functions framework
...
const hashJson = (jsonString: string) => {
...
const getJobs = async () => {
  ...
  const jobs: AmazonJob[] = await page.$$eval('.job-card-module_root__QYXVA', elements =>
    elements.map((el: any) => {
...
const saveJobsHash = (hash: string) => {
...
const notifyUser = (goodMessage: boolean)=>{
  const mailOptions = {
...
const driverProgram = async () => {
  const jobs: AmazonJob[] = await getJobs();
